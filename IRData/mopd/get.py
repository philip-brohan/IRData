...
def load_simple(
    ...
    file_name = make_local_file_name(
        dataset_name,
        fdate.year,
        fdate.month,
        fdate.day,
    ...
    if not os.path.isfile(file_name):
        if auto_fetch:
            ...
            raise StandardError(
                "No Data for %s, realisation %d, forecast %d"
                % (vdate, realization, forecast_period)
                + " on disc."
            )
    # Get the field from the file - want one within 2 minutes
    hslice = load_specific(
        dataset_name, variable, file_name, year, month, day, ihour, iminute
    )
    return hslice
...
def convert_variable_name(dataset_name, variable):
    ...
    raise StandardError("No variable %s, in dataset %s" % (variable, dataset_name))
...
def get_forecast_date_from_validity_date(dataset_name, vdate, forecast_period):
    ...
    if dataset_name == "mogreps-uk":
        ...
        return fdate
    raise StandardError(
        "Unsupported dataset %s. " % dataset_name + "Must be 'mogreps-g or mogreps-uk"
    )


def is_single_field(
    dataset_name, variable, year, month, day, hour, realization, forecast_period
):
...
def validate_dataset_name(dataset_name):
    ...
    if validated_name != "mogreps-g" and validated_name != "mogreps-uk":
        raise StandardError(
            "Unsupported dataset %s. " % dataset_name
...
def is_file_for(dataset_name, year, month, day, hour, realization, forecast_period):
    ...
    if dataset_name == "mogreps-g":
        ...
        if realization < 0 or realization > 11:
            return False
        if forecast_period % 3 != 0 or forecast_period < 3 or forecast_period > 174:
            return False
        return True
    if dataset_name == "mogreps-uk":
        ...
        if realization < 0 or realization > 22:
            return False
        if forecast_period % 3 != 0 or forecast_period < 3 or forecast_period > 36:
            return False
        return True
    raise StandardError(
        "Unsupported dataset %s. " % dataset_name + "Must be mogreps-g or mogreps-uk"
...
def make_local_file_name(
    ...
    file_name = template_string.format(
        os.environ["SCRATCH"], dataset_name, year, month, day, hour
    )
    template_string = "prods_op_{}_{:04d}{:02d}{:02d}_{:02d}_{:02d}_{:03d}.nc"
    file_name = file_name + template_string.format(
        dataset_name, year, month, day, hour, realization, forecast_period
    )
    return file_name
...
def fetch_data(dataset_name, year, month, day, hour, realization, forecast_period):
    ...
    if not is_file_for(
        ...
        raise StandardError(
            "No data file at hour %d, realisation %d, forecast %d"
    ...
    os.system("wget -P %s %s" % (os.path.dirname(local_file_name), remote_url))


def load_specific(dataset_name, variable, file_name, year, month, day, hour, minute=0):
    ...
    if dataset_name == "mogreps-g":
        if variable == "prmsl":
            if (
                re.search("03.nc", file_name) != None
            ):  # 3-hour data includes multiple times
                time_constraint = iris.Constraint(
                    time=datetime_to_float("mogreps-g", vdate)
                )
                variable_constraint = iris.Constraint(name="air_pressure_at_sea_level")
                hslice = iris.load_cube(
                    file_name, time_constraint & variable_constraint
                )
                return hslice
            else:
                variable_constraint = iris.Constraint(name="air_pressure_at_sea_level")
    ...
    if dataset_name == "mogreps-uk":
        if variable == "prmsl":
            time_constraint = iris.Constraint(
                time=datetime_to_float("mogreps-uk", vdate)
            )
            variable_constraint = iris.Constraint(name="air_pressure_at_sea_level")
            hslice = iris.load_cube(file_name, time_constraint & variable_constraint)
            return hslice
        if variable == "air.2m":
            time_constraint = iris.Constraint(
                time=datetime_to_float("mogreps-uk", vdate)
            ...
            variable_constraint = iris.Constraint(name="air_temperature")
            hslice = iris.load(file_name, time_constraint & variable_constraint)
            return hslice[3]
        if variable == "uwnd.10m":
            time_constraint = iris.Constraint(
                time=datetime_to_float("mogreps-uk", vdate)
