...
def _get_slice_at_hour_at_timestep(
    variable, year, month, day, hour, fc_init=None, deaccumulate=True
):
    ...
    if (
        variable == "prate"
        and deaccumulate
        and (hour != 21 or (fc_init is not None and fc_init == "last"))
    ):
        r1 = _get_slice_at_hour_at_timestep(
            variable, year, month, day, hour, fc_init=fc_init, deaccumulate=False
        )
        # Subtract the values from 3 hours ago
        lt = datetime.datetime(year, month, day, int(hour)) - datetime.timedelta(
            hours=3
        )
        r2 = _get_slice_at_hour_at_timestep(
            variable,
            lt.year,
            lt.month,
            lt.day,
    ...
    file_name = _hourly_get_file_name(
        variable, year, month, day, hour, type=type, fc_init=fc_init
    )
    if not os.path.isfile(file_name):
        raise Exception(
            ("%s for %04d/%02d not available" + " might need cera20c.fetch")
            % (variable, year, month)
        )
...
def load(variable, dtime, fc_init=None):
    ...
    Args:
        ...
        dtime (:obj:`datetime.datetime`): Date-time to get data for.
    ...
    if _is_in_file(variable, dtime.year, dtime.month, dtime.day, dhour):
        return _get_slice_at_hour_at_timestep(
            variable, dtime.year, dtime.month, dtime.day, dhour, fc_init=fc_init
        )
