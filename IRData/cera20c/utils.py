...
def _hourly_get_file_name(
    variable, year, month, day=15, hour=12, fc_init=None, type="ensemble"
):
    base_dir = _get_data_dir()
    if type == "normal":
        file_name = "%s/normals/hourly/%02d/%s.nc" % (base_dir, month, variable)
        return file_name

    if type == "standard.deviation":
        file_name = "%s/standard.deviations/hourly/%02d/%s.nc" % (
            base_dir,
            month,
            variable,
        )
    ...
    dir_name = "%s/hourly/%04d/%02d/" % (base_dir, year, month)
    ...
    name = "%s/hourly/%04d/%02d/%s.nc" % (base_dir, year, month, variable)
    return name
