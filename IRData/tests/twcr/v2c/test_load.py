...
def fake_cube(file_name, constraint):
    ...
    ensemble = iris.coords.DimCoord(numpy.linspace(0, 9, 10), long_name="member")
...
class TestLoad(unittest.TestCase):
    ...
    def test_load_prmsl(self):
        ...
        with patch.object(iris, "load_cube", side_effect=fake_cube) as mock_load:
            tc = twcr.load("prmsl", datetime.datetime(2010, 3, 12, 6), version=version)
        # Right dimensions
    ...
    def test_load_prmsl_interpolated(self):
        fake_data_file("prmsl", version, 2010)
        with patch.object(iris, "load_cube", side_effect=fake_cube) as mock_load:
            tc = twcr.load("prmsl", datetime.datetime(2010, 3, 12, 9), version=version)
        # Right dimensions
        self.assertEqual(len(tc.coords()), 4)
        # Right ensemble dimension name?
        self.assertEqual(tc.coords()[0].long_name, "member")
    ...
    def test_fetch_mslp(self):
        with self.assertRaises(Exception) as cm:
            tc = twcr.load("mslp", datetime.datetime(1969, 3, 12), version=version)
        self.assertIn(
            "One or more of the files specified did not exist", str(cm.exception)
        )
