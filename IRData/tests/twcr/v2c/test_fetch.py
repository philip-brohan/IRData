...
class TestFetch(unittest.TestCase):
    ...
    def test_fetch_prmsl(self):
        with patch.object(subprocess, "call", return_value=0) as mock_method:
            twcr.fetch("prmsl", datetime.datetime(1969, 3, 12), version=version)
        remote_dir = (
        ...
        mock_method.assert_called_once_with(
            "wget -O %s %s" % (local_file, remote_file), shell=True
        )
    ...
    def test_fetch_prmsl_eol(self):
        ...
        with patch.object(subprocess, "call", return_value=0) as mock_method:
            twcr.fetch("prmsl", datetime.datetime(1969, 12, 31, 22), version=version)
        remote_file = "%s/analysis/prmsl/prmsl_1969.nc" % remote_dir
        local_file = "%s/1969/prmsl.nc" % local_dir
    ...
    def test_fetch_observations(self):
        remote_file = (
            "http://portal.nersc.gov/project/m958/2c_observations/" + "1969.zip"
        )
        local_dir = "%s/20CR/version_%s/" % (os.environ["SCRATCH"], version)
        ...
        with patch.object(subprocess, "call", return_value=0) as mock_get:
            with self.assertRaises(IOError) as cm:
                twcr.fetch_observations(datetime.datetime(1969, 3, 12), version=version)
        ...
        tzf.close()
        with patch.object(
            zipfile.ZipFile, "extractall", return_value=None
        ) as mock_unzip:
            twcr.fetch_observations(datetime.datetime(1969, 3, 12), version=version)
        mock_unzip.assert_called_once_with("%s/observations/" % local_dir)
