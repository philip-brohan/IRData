...
def fake_data_file(version, year, month, day, hour):
    file_name = (
        "%s/20CR/version_%s/observations/%04d/prepbufrobs_assim_%04d%02d%02d%02d.txt"
        % (os.environ["SCRATCH"], version, year, year, month, day, hour)
    )
...
def fake_prfwf(file_name, **kwargs):
    ...
    for idx in range(len(offsets)):
        ...
        uid.append(
            "%04d%02d%02d%02dxxxxxxxxxx" % (dtt.year, dtt.month, dtt.day, dtt.hour)
        )
    dd = {}
...
class TestLoad(unittest.TestCase):
    ...
    def test_load_1file(self):
        ...
        with patch.object(pandas, "read_fwf", side_effect=fake_prfwf) as mock_load:
            o = twcr.load_observations_1file(
                datetime.datetime(1987, 7, 2, 6), version=version
            )
        # Right dimensions
    ...
    def test_load_1file_weight(self):
        ...
        with patch.object(pandas, "read_fwf", side_effect=fake_prfwf) as mock_load:
            o = twcr.load_observations_fortime(
                datetime.datetime(1987, 7, 2, 7, 30), version=version
            )
        # Right dimensions
        self.assertEqual(len(o.weight), 40)
        # Closer to 6 than 12
        expected = numpy.concatenate((numpy.repeat(0.75, 20), numpy.repeat(0.25, 20)))
        self.assertTrue((o.weight == expected).all())
    ...
    def test_fetch_missing(self):
        ...
        self.assertIn("No obs file for given version and date", str(cm.exception))
