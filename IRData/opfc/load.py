        ...
        if variable == "prate_a":
            if validity_time.minute == 30:
                ...
                if validity_time.minute < 30:
                    ...
                    prevt = validity_time - datetime.timedelta(
                        minutes=validity_time.minute - 30
                    )
...
def _get_fcst_period(variable, validity_time, model="global", methods=None):
    ...
    fp = validity_time - _get_fcst_reference_time(
...
def _get_slice_at_hour_at_timestep(
    variable, validity_time, methods=None, model="global"
):
    ...
    if not os.path.isfile(file_name):
        raise Exception(
            ("Data for %04d/%02d/%02d not available" + " might need oper.fetch")
            % validity_time.strftime("%Y-%m-%d")
        )
    if variable == "prate_a":
        ...
        ftco = iris.Constraint(
            forecast_period=lambda t: t.bound is not None
            and t.bound[0] == tp[0]
            and t.bound[1] == tp[1]
        )
    else:
        ftco = iris.Constraint(
            forecast_period=_get_fcst_period(
                variable, validity_time, model=model, methods=methods
            ).seconds
            // 3600
        )
    ftfr = iris.Constraint(
        forecast_reference_time=_get_fcst_reference_time(
            variable, validity_time, model=model, methods=methods
        )
    )
...
def load(variable, validity_time, methods=None, model="global"):
    ...
    if len(ftimes) == 1:
        return _get_slice_at_hour_at_timestep(
            variable, ftimes[0], methods=methods, model=model
        )
    s_previous = _get_slice_at_hour_at_timestep(
        variable, ftimes[0], methods=methods, model=model
    )
    s_next = _get_slice_at_hour_at_timestep(
        variable, ftimes[1], methods=methods, model=model
    )
